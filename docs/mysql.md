...
[MVCC详解，深入浅出简单易懂-CSDN博客](https://blog.csdn.net/lans_g/article/details/124232192)

```markdown
1、基于实现 - undolog、版本链、readview
2、快照读&当前读
...
```markdown
MySQL MGR（MySQL Group Replication）是MySQL官方提供的一种高可用性解决方案，用于构建具有自动故障转移和数据复制能力的集群。
它基于多主复制技术，允许多个MySQL节点以组的形式协同工作，实现数据的同步和故障切换。

以下是MySQL MGR集群的一些关键特点和优势：

1. 高可用性： 
MGR通过自动检测节点的可用性并在主节点故障时自动进行主从切换，从而实现高可用性。
如果主节点发生故障，系统会自动将一个从节点提升为新的主节点，确保数据库持续可用。

2. 数据一致性：
MGR通过多主复制技术确保各个节点之间的数据一致性。
数据写入主节点后，会自动同步到其他节点，从而保持数据的一致性。

3. 节点自动管理：
MGR可以自动检测节点的加入和退出，节点之间的状态变化等，并根据配置进行相应的操作。
这样，当有新的节点加入或旧的节点退出时，集群可以自动适应变化。

4. 读写分离：
MGR支持在不同节点上进行读写分离，从而分担主节点的读取负载，提高性能。

5. 弹性伸缩：
由于MGR支持节点的动态加入和退出，因此可以根据需求进行水平扩展或收缩，以适应业务的变化。

6. 容错性：
MGR的复制机制能够容忍节点间的网络故障、节点崩溃等问题，保证数据的连续性和一致性。

需要注意的是，虽然MySQL MGR提供了许多优点，但也需要根据实际情况来评估是否适合你的应用场景。

在部署MGR集群时，需要考虑网络稳定性、硬件资源、数据量等因素，以及合适的监控和维护策略，以确保集群的可用性和性能。
